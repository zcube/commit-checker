...
| **セマンティックバージョン計算** | Gitの履歴からSemVerを計算（GitVersion連携、設定の直接内埋め込み対応） |
...
# semver コマンド設定（省略可）
# semver:
#   gitversion:                # GitVersion設定を直接内埋め込み（GitVersion.yml の自動探索より優先）
...
commit-checker semver        Gitの履歴からセマンティックバージョンを計算
...
[GitVersion](https://gitversion.net/) 設定を使って、Gitの履歴からセマンティックバージョンを計算します。

```bash
# 現在のディレクトリのSemVerを出力（デフォルト）
commit-checker semver

# フルSemVer（プレリリース + ビルドメタデータ含む）
...
**GitVersion設定の内埋め込み（`semver.gitversion`）**

`.commit-checker.yml` に GitVersion 設定を直接内埋め込みできます。
内埋め込み設定がある場合は、リポジトリルートの `GitVersion.yml` の自動探索より優先されます。
